...
from core.models.article_outline import ArticleOutline, OutlineSection
...
class ResearchCrew:
    ...
    async def research_topic(
        ...
        Args:
            ...
            content_type: 内容类型ID（如"news"、"blog"等）
        ...
        if isinstance(topic, str):
            topic_title = topic
            logger.info(f"开始研究话题: {topic_title}, 内容类型: {content_type or '未指定'}")
        else:
            # 假设是 Topic 对象
            ...
            # 如果 content_type 未指定，尝试从 Topic 获取
            if not content_type and hasattr(topic, 'content_type'):
        ...
        if content_type:
            ...
            if content_type_obj:
                logger.info(f"获取到内容类型配置: {content_type_obj.name}")

                # 如果未指定深度，根据内容类型的research_intensity确定深度
                if not depth and hasattr(content_type_obj, 'research_level'):
                    research_level = content_type_obj.research_level
                    if research_level <= 1:
                        depth = "shallow"
                        ...
                        depth = "medium"
                    logger.info(f"根据内容类型设置研究深度: {depth}")
        ...
        # 如果有内容类型对象，将其信息添加到研究配置中
        if content_type_obj:
            self.current_research_config["content_type_info"] = content_type_obj.get_type_summary()
        ...
        try:
            ...
            background_task = self._create_background_research_task(topic_title, content_type)
            ...
            if needs_expert:
                ...
                expert_task = self._create_expert_finder_task(
                    topic_title, background_result[0], content_type
                )
            ...
            research_result = self._process_research_results(
                ...
                topic_id,
                topic if hasattr(topic, 'id') else None
            )
            ...
            # 增加任何附加选项到元数据
            if options:
                research_result.metadata.update(options)

            # 如果有话题ID，添加到元数据中
            if topic_id:
                research_result.metadata["topic_id"] = topic_id
            ...
            basic_research = self._workflow_result_to_basic_research(research_result)

            # 如果有topic_id，返回TopicResearch，否则返回BasicResearch
            if topic_id:
    ...
    async def run_full_workflow(
        ...
        logger.info(f"开始研究工作流，话题: {topic_title}, 内容类型: {content_type or '未指定'}")
    ...
    def _process_research_results(self, topic_title, workflow_result, background_research, expert_insights,
                             data_analysis, final_report, topic_id=None, topic_obj=None):
        """处理研究结果，整合所有部分的结果"""
        ...
        if "research_config" in result.metadata:
            ...
            # 确保使用最初传入的研究配置中的content_type_info
            if "content_type" in result.metadata["research_config"] and "content_type_info" not in result.metadata["research_config"]:
                content_type = result.metadata["research_config"]["content_type"]
                # 尝试从ContentManager获取content_type_info
                content_type_obj = ContentManager.get_content_type(content_type)
                if content_type_obj:
                    result.metadata["research_config"]["content_type_info"] = content_type_obj.get_type_summary()
