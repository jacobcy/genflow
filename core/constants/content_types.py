...
# 内容类型分组
SHORT_CONTENT_TYPES = [CONTENT_TYPE_FLASH_NEWS, CONTENT_TYPE_SOCIAL_MEDIA]
STANDARD_CONTENT_TYPES = [
    CONTENT_TYPE_NEWS, CONTENT_TYPE_BLOG, CONTENT_TYPE_TUTORIAL,
    CONTENT_TYPE_REVIEW, CONTENT_TYPE_STORY, CONTENT_TYPE_TECH,
    CONTENT_TYPE_QA, CONTENT_TYPE_ENTERTAINMENT, CONTENT_TYPE_LIFE,
    CONTENT_TYPE_SCIENCE
]
DEEP_CONTENT_TYPES = [CONTENT_TYPE_PAPER, CONTENT_TYPE_RESEARCH_REPORT, CONTENT_TYPE_ANALYSIS]

# 所有支持的内容类型
ALL_CONTENT_TYPES = SHORT_CONTENT_TYPES + STANDARD_CONTENT_TYPES + DEEP_CONTENT_TYPES
...
def get_content_type_group(content_type: str) -> str:
    ...
    Returns:
        str: 分组名称("短内容", "标准内容", "深度内容")
    """
    if content_type in SHORT_CONTENT_TYPES:
        return "短内容"
    elif content_type in STANDARD_CONTENT_TYPES:
        return "标准内容"
    elif content_type in DEEP_CONTENT_TYPES:
        return "深度内容"
    else:
        return "未知"

def get_research_config(content_type: Optional[str] = None) -> Dict:
    """获取特定内容类型的研究配置

    Args:
        content_type: 内容类型，如果为None则返回默认配置

    Returns:
        Dict: 研究配置字典
    """
    if not content_type or content_type not in RESEARCH_CONFIG:
        return DEFAULT_RESEARCH_CONFIG.copy()
    return RESEARCH_CONFIG[content_type].copy()

def get_writing_config(content_type: Optional[str] = None) -> Dict:
    """获取特定内容类型的写作配置
    ...
    Returns:
        Dict: 写作配置字典
    """
    if not content_type or content_type not in WRITING_CONFIG:
        return DEFAULT_WRITING_CONFIG.copy()
    return WRITING_CONFIG[content_type].copy()

def get_content_types_by_depth(depth: str) -> List[str]:
    """获取特定研究深度级别的所有内容类型
    ...
    return [
        content_type
        for content_type, config in RESEARCH_CONFIG.items()
...
def get_content_type_from_category(category: str) -> Optional[str]:
    """根据平台分类标签获取推荐的内容类型
    ...
    Returns:
        Optional[str]: 推荐的内容类型，如果没有匹配则返回None
    """
    return CATEGORY_TO_CONTENT_TYPE.get(category)

def get_content_type_from_categories(categories: List[str]) -> Optional[str]:
    """根据平台分类标签列表获取最匹配的内容类型

    当有多个分类标签时，按优先级选择最合适的内容类型：
    1. 尝试找出在类别映射中出现次数最多的内容类型
    2. 如果有相同次数的，优先选择深度内容 > 标准内容 > 短内容
    ...
    # 仅有一个分类时直接返回
    if len(categories) == 1:
        return get_content_type_from_category(categories[0])

    # 统计每个内容类型的匹配次数
    content_type_counts = {}
    for category in categories:
        content_type = get_content_type_from_category(category)
        if content_type:
    ...
    most_frequent = [
        content_type for content_type, count in content_type_counts.items()
        if count == max_count
    ]
    ...
    # 如果有多个相同频率的内容类型，按深度优先级返回
    for content_type in most_frequent:
        if content_type in DEEP_CONTENT_TYPES:
            return content_type

    for content_type in most_frequent:
        if content_type in STANDARD_CONTENT_TYPES:
            return content_type
