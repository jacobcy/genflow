...
class CrewAIManagerController(Flow[ManagerProductionState]):
    ...
    def execute_crew(self, _):
        ...
        try:
            ...
            # 2. 创建任务
            topic_task = self._create_topic_task(self.state.category, self.state.keywords)
    ...
    def _create_topic_task(self, category: str, keywords: Optional[List[str]] = None) -> Task:
        """创建选题任务"""
    ...
    def _create_writing_task(self, style: Optional[str] = None) -> Task:
        ...
        content_type_info = ""
        if hasattr(self.state, 'content_type') and self.state.content_type:
