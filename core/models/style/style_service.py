...
class StyleFactory:
    ...
    def match_existing_style(text: str) -> Optional[Any]:
        ...
        for style in all_styles:
            if style.name.lower() == text_lower:
                return style

        # 2. 模糊匹配 - 检查名称、描述和类型
        for style in all_styles:
            # 检查风格名称
            if text_lower in style.name.lower():
                return style

            # 检查风格描述
            if style.description and text_lower in style.description.lower():
                return style

            # 检查风格类型
            if style.type and text_lower in style.type.lower():
                return style
